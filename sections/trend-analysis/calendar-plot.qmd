...
#| fig-cap: '2003年 O~3~ 污染日历图 `calendarPlot` for O~3~ concentrations in 2003.'
#| fig-width: 6
#| fig-height: 8
library(openair)
...
#| fig-cap: '2003年 PM~10~ 污染日历图，显示 PM~10~ 浓度值，并对大于50微克/立方米的天高亮显示。'
#| fig-width: 8
#| fig-height: 6
calendarPlot(mydata,
...
#| fig-cap: '2003年 O~3~ 污染日历图。使用风向箭头作为每日的注释，箭头的长短代表风速的大小。可以看到图中四月18和19日臭氧的重污染都发生在比较强的东北风条件下。'
#| fig-height: 8
#| fig-width: 6
#| out-width: 80%
calendarPlot(mydata,
...
#| fig-cap: '`2003年 NO~2~ 浓度日历图，使用用户自定义浓度段进行填色。'
#| fig-width: 8
#| fig-height: 6
calendarPlot(mydata,
...
下面的几个例子展示了如何针对不同污染物使用不同的平均时间。通过 `importAURN` 和类似的几个函数下载得到的数据中污染物浓度的单位应该是μg m^-3^，如果不是的话需要做相应的单位换算。默认情况下，日历图中显示的数字是这一天的日期，可以通过 `annotate = "value"`显示这一天的浓度。这样就可以将污染物浓度的具体数值和对应的级别填色*同时*展示出来了。
