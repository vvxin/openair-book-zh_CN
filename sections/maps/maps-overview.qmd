...
[openairmaps]{.pkg} 包发布在 CRAN 官方软件残酷，所以可以通过 `install.packages()` 简单的完成下载安装。
...
如果希望尝试还未正式发布的新功能和错误修复，可以从 GitHub 下载安装 [openairmaps]{.pkg} 的开发版本。
...
在使用[openairmaps]{.pkg} 进行分析之前，别忘了先通过`library()`将之加载到运行环境当中。
...
本章使用R `r getRversion()` 版本和 [openairmaps]{.pkg} `r packageDescription("openairmaps", field = "Version")`版本制作生成。
:::
...
随着R语言生态系统的不断发展，`{rmarkdown}`和最近的[Quarto](https://quarto.org/)
