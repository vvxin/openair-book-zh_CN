...
# 获取英国空气质量数据 {#sec-importAURN}

## 获取监测数据

英国有大量可公开访问的空气质量数据。其中最重要的几个大型数据库都可以免费访问，包括英国AURN全国数据库、分区（英格兰、苏格兰、威尔士和北爱尔兰）数据库以及伦敦帝国理工学院的伦敦空气质量网等。以上这几个大型数据库都可以保证统一的数据格式和高标准的数据质量。

[openair]{.pkg} 提供一系列从这些数据库获取英国空气质量监测数据的函数。里卡多能源与环境咨询公司还制作了包含英国几大空气质量监测网数据的.RData 文件（R工作区），并以日为单位持续更新。非常感谢里卡多能源与环境咨询公司的Trevor Davies为此所作的大量工作。这种数据访问方式需要互联网公网环境才能工作。可访问的网络包括： 

-   `importAURN` 用于获取英国国家网数据 [AURN 城乡自动监测网](https://uk-air.defra.gov.uk/networks/network-info?view=aurn)，这是英国最主要的监测网络。

-   `importSAQN` 用于获取 [苏格兰空气质量](http://www.scottishairquality.scot/) 监测网数据。

-   `importWAQN` 用于获取 [威尔士空气质量](https://airquality.gov.wales/) 监测网数据。

-   `importAQE` 用于获取 [英格兰空气质量](https://www.airqualityengland.co.uk/) 监测网站点的数据。

-   `importNI` 用于获取 [北爱尔兰空气质量](https://www.airqualityni.co.uk) 监测网络站点的数据。

-   `importLocal` 用于获取英国境内地方管理的空气质量监测网的数据。地方站点和严格按照技术规定组织监测的国家网站点不同，其监测目的、点位选址、监测项目和数据质量要求可能都会有一些差异，其中大部分是地方政府为了监测工业区、机场或者其它项目的要求建设运行的。更多信息请访问[这里](https://uk-air.defra.gov.uk/networks/network-info?view=nondefraaqmon)。这个数据源的数据大概包含了15个不同的地方空气质量监测网。

-   `importEurope` 是Stuart Grange的 [saqgetr]{.pkg} 包————见 <https://github.com/skgrange/saqgetr>的一个简化版本，用于访问欧盟小时数据。[openair]{.pkg} 版本的函数保持了与 [openair]{.pkg} 其它数据获取函数类似的参数要求，如需要输入站点编码和年份。

-   `importKCL` 用于获取由伦敦帝国理工大学[^uk-air-quality-data-1]运行的站点的数据，主要包括的是[伦敦市空气质量监测网](https://www.londonair.org.uk/LondonAir/Default.aspx).
...
多数用户会通过空气质量数据库的官网 <https://www.airquality.co.uk> 来下载空气质量小时数据。数据会以固定的格式将 .csv 文件用过电子邮件发给用户。这种方法可行但是同时也有一些局限性，我们将在下文介绍一种新的数据下载和存储方式。
...
接下来就可以用 `importAURN` 函数来获取数据了。下面展示了几种不同形式的用法。
...
所有监测数据获取类的函数默认返回小时数据，但是很多时候我们需要的可能只是年均值。为此英国监测数据获取类函数（除`importKCL`）添加了可以返回不同平均时间的数据，如年均、月均、日均以及 SO~2~ 的15分钟平均等。其中年均和月均的数据还包含了数据比较重要的数据捕获率的信息。这个平均时间由 `data_type` 选项，可以选择的值如下：
